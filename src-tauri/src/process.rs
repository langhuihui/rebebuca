    ...
    SHELL_ENV_CACHE.get_or_init(|| {
        ...
        if let Ok(output) = result {
            if output.status.success() {
                ...
                println!("[PROCESS] Loaded {} environment variables from login shell (cached)", env_map.len());
            }
        }
        ...
        if env_map.is_empty() || !env_map.contains_key("PATH") {
            ...
            println!("[PROCESS] Using fallback PATH: {}", env_map.get("PATH").unwrap());
        }
...
fn get_shell_env() -> HashMap<String, String> {
    ...
    // Return empty map - we'll rely on std::env::vars() to get current environment
    HashMap::new()
}
