  ...
  <div class="terminal-wrapper" :class="{ 'drag-over': isDragOver }">
    ...
    <div v-if="showSearch" class="terminal-search-bar">
      ...
      <span v-if="searchResultCount !== null" class="search-count">
        {{ searchResultIndex !== null ? searchResultIndex + 1 : 0 }}/{{
          searchResultCount
        }}
      </span>
      <button
...
const initTerminal = async () => {
  if (!terminalContainer.value || isInitialized) {
    console.log(
      "[TerminalView] Skipping initTerminal - container:",
  ...
  terminal = new Terminal({
    ...
    fontSize: 14,
    fontFamily:
      '"Cascadia Code", "Fira Code", "Source Code Pro", Menlo, Monaco, "Courier New", monospace',
  ...
  // Setup shell integration (for interactive shells). This is independent from PTY creation.
  if (props.enableShellIntegration) {
  ...
  if (!terminalContainer.value) {
    console.warn(
      "[TerminalView] Container element not found, aborting terminal initialization",
    );
    terminal?.dispose();
    terminal = null;
    return;
    ...
    try {
      ...
      webglAddon.onContextLoss(() => {
        ...
        try {
          ...
          console.warn(
            "[Terminal] Canvas fallback failed, using DOM renderer:",
            canvasError,
          );
        }
      ...
      console.warn(
        "[Terminal] WebGL not available, trying canvas renderer:",
        e,
      );
      // Fallback to canvas
      try {
        ...
        console.warn(
          "[Terminal] Canvas renderer not available, using DOM renderer:",
          canvasError,
        );
      }
    }
  ...
  try {
    ...
    terminal.onResize(({ cols, rows }) => {
      adapterInstance.terminal
        .resize(props.ptyId, cols, rows)
        .catch((err: Error) => {
          // Ignore "PTY not found" errors - this happens when process exits quickly
          const errStr = String(err);
          if (
            errStr.includes("PTY not found") ||
        ...
        entries.forEach((entry) => {
          if (entry.isIntersecting && entry.intersectionRatio > 0) {
            ...
            if (wasHidden && terminal) {
              console.log(
                "[TerminalView] Element became visible (via IntersectionObserver), restoring renderer",
              );
              wasHidden = false;
        ...
        mutations.forEach((mutation) => {
          if (
            ...
            if (isVisible && wasHidden && terminal) {
              console.log(
                "[TerminalView] Element became visible (via MutationObserver), restoring renderer",
              );
              wasHidden = false;
              restoreRenderer();
            } else if (!isVisible) {
...
const restoreRenderer = () => {
  if (!terminal || !terminalContainer.value) {
    console.warn(
      "[TerminalView] Cannot restore: terminal or container not available",
    );
    return;
  }
  ...
  console.log(
    "[TerminalView] Terminal buffer active length:",
    terminal.buffer.active.length,
  );
  console.log(
    "[TerminalView] Terminal buffer normal length:",
    terminal.buffer.normal.length,
  );

  // Check if terminal has any content
  const hasContent =
    terminal.buffer.active.length > 0 || terminal.buffer.normal.length > 0;
  ...
  if (hasContent) {
    ...
    try {
      ...
      if (activeBuffer.length > 0) {
        ...
        const lastLine = activeBuffer.getLine(
          Math.min(activeBuffer.length - 1, terminal.rows - 1),
        );
        console.log(
          "[TerminalView] First line content:",
          firstLine?.translateToString(true),
        );
        console.log(
          "[TerminalView] Last line content:",
          lastLine?.translateToString(true),
        );
      }
    } catch (e) {
  ...
  nextTick(() => {
    ...
    try {
      fitAddon.fit();
      console.log(
        "[TerminalView] Terminal fitted, cols:",
        terminal.cols,
        "rows:",
        terminal.rows,
      );
    ...
    setTimeout(() => {
      ...
      try {
        if (terminal.rows > 0) {
          ...
          try {
            ...
            if (renderer) {
              ...
              if (renderer._renderService) {
                ...
                if (renderService.onRequestRefreshRows) {
                  renderService.onRequestRefreshRows.fire({
                    start: 0,
                    end: terminal.rows - 1,
                  });
                }
            ...
            console.warn(
              "[TerminalView] Could not access renderer directly:",
              rendererError,
            );
          }
          ...
          console.log(
            "[TerminalView] Terminal refreshed using multiple methods",
          );
        }
      } catch (e) {
      ...
      setTimeout(() => {
        if (terminal && fitAddon) {
          ...
          console.log(
            "[TerminalView] Final buffer length after restore:",
            terminal.buffer.active.length,
          );
        }
      }, 100);
...
const takeScreenshot = async (): Promise<string | null> => {
  if (!terminal || !terminalContainer.value) {
    console.warn(
      "[TerminalView] Cannot take screenshot: terminal or container not available",
    );
    return null;
  }
  ...
  try {
    ...
    if (!xtermElement) {
      console.warn(
        "[TerminalView] Cannot take screenshot: xterm element not available",
      );
      return null;
    }
    ...
    if (!ctx) {
      console.warn(
        "[TerminalView] Cannot take screenshot: failed to get 2d context",
      );
      return null;
    }
...
watch(
  ...
  async (newPtyId, oldPtyId) => {
    if (newPtyId && oldPtyId && newPtyId !== oldPtyId) {
      ...
      const isPtyIdUpdate =
        oldPtyId.startsWith("task-") && terminal && isInitialized;
      ...
      if (isPtyIdUpdate && hasContent) {
        ...
        console.log(
          "[TerminalView] PTY ID updated from placeholder to actual:",
          oldPtyId,
          "->",
          newPtyId,
        );
        console.log(
          "[TerminalView] Keeping existing terminal with content, buffer length:",
          terminal?.buffer?.active?.length,
        );
        return;
      }
      ...
      console.log(
        "[TerminalView] PTY ID changed (real restart), reinitializing:",
        oldPtyId,
        "->",
        newPtyId,
      );
      console.log(
        "[TerminalView] Buffer length before dispose:",
        terminal?.buffer.active.length,
      );
      ...
      console.log(
        "[TerminalView] PTY ID unchanged during tab switch:",
        newPtyId,
        "terminal exists:",
        !!terminal,
      );
...
onUnmounted(() => {
  console.log(
    "[TerminalView] Component unmounted for ptyId:",
    props.ptyId,
    "buffer length before dispose:",
    terminal?.buffer.active.length,
  );
  if (unlistenDragDrop) {
    unlistenDragDrop();
...
.terminal-search-bar {
  ...
  border-bottom: 1px solid
    v-bind('props.theme === "dark" ? "#404040" : "#d0d0d0"');
