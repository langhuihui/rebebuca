<template>
  <div class="ffmpeg-config-panel">
    <n-card title="FFmpeg 视频编码" :bordered="false">
      <!-- 模式切换 -->
      ...
      <div class="section">
        <div class="section-header">
          <h3>输入文件</h3>
          <n-space>
            <n-button @click="handleAddFiles">
              <template #icon>
                <n-icon><AddIcon /></n-icon>
              </template>
              添加文件
            </n-button>
            <n-button @click="handleClearFiles" :disabled="!hasInputFiles">
              清空列表
            </n-button>
          </n-space>
        </div>
        ...
        >
          <n-empty description="拖拽文件到此处或点击添加文件">
            <template #icon>
              <n-icon :size="64"><FolderIcon /></n-icon>
            </template>
          </n-empty>
        </div>
        ...
        <div class="file-list" v-else>
          <n-list bordered>
            <n-list-item v-for="(file, index) in inputFiles" :key="index">
              ...
              <n-ellipsis :tooltip="{ content: file.path }">
                {{ file.name }}
              </n-ellipsis>
              <template #suffix>
                <span class="file-size">{{ formatFileSize(file.size) }}</span>
                <n-button text @click="handleRemoveFile(index)">
                  <template #icon>
          ...
          <div class="batch-mode-indicator" v-if="isBatchMode">
            <n-tag type="info">批量模式: {{ inputFiles.length }} 个文件</n-tag>
          </div>
        </div>
      ...
      <div class="action-buttons">
        <n-space justify="center">
          ...
          <n-button @click="handleSavePreset">
            <template #icon>
