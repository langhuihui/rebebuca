...
export const useAICollabNativeStore = defineStore('aiCollabNative', () => {
  ...
  const handleStreamEvent = (aiSessionId: string, event: TypedStreamEvent) => {
    ...
    switch (event.type) {
      ...
      case 'usage':
        // 累加 usage，并触发响应式更新
        console.log('[AICollabNative] Usage event received:', {
          sessionId: session.id,
          eventUsage: event.usage,
          currentUsage: session.usage,
        });
        session.usage = {
          promptTokens: session.usage.promptTokens + (event.usage?.promptTokens || 0),
          completionTokens: session.usage.completionTokens + (event.usage?.completionTokens || 0),
          totalTokens: session.usage.totalTokens + (event.usage?.totalTokens || 0),
        };
        session.lastActivity = Date.now();
        // 触发 Map 更新（通过重新设置来确保响应式）
        sessions.value.set(session.id, { ...session });
        console.log('[AICollabNative] Updated usage:', session.usage);
        // 保存 usage 更新
        debouncedSaveSession(session);
        break;
  ...
  // 保存会话的防抖函数
  const saveSessionDebounce = new Map<string, ReturnType<typeof setTimeout>>();

  const debouncedSaveSession = (session: NativeCollabSession) => {
    // 清除之前的定时器
    const existingTimer = saveSessionDebounce.get(session.id);
    if (existingTimer) {
    ...
    const timer = setTimeout(async () => {
      ...
      saveSessionDebounce.delete(session.id);
    }, 500);

    saveSessionDebounce.set(session.id, timer);
  };
  ...
  const addMessageToSession = (
    ...
    // 如果是 worker 消息，尝试更新现有未完成的 worker 消息而不是创建新消息
    if (message.from === 'worker') {
      // 查找最近的未完成的 worker 消息（流式或最近创建的）
      const existingMsg = session.messages
        .slice()
        .reverse()
        .find(
          m => m.from === 'worker' &&
            (m.type === 'streaming' || !m.metadata?.isComplete) &&
            Date.now() - m.timestamp < 5000 // 5秒内的消息才认为是同一个流
        );

      if (existingMsg) {
        // 更新现有消息的内容（追加）
        const newContent = typeof message.content === 'string' ? message.content : JSON.stringify(message.content);
        existingMsg.content = existingMsg.content + newContent;
        ...
        session.lastActivity = existingMsg.timestamp;
  ...
  const loadSession = async (projectPath: string): Promise<NativeCollabSession | null> => {
    ...
    try {
      ...
      const session: NativeCollabSession = {
        ...
        toolCallCount: sessionData.toolCallCount || 0, // 确保有默认值
      };
